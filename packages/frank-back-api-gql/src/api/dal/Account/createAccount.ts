...
export default createMutation<Args, Account>(
  ...
  async (args, { db }) => {
    ...
    const columns = [account.teamId, account.name, account.data, account.currencyCode]

    const values = [args.teamId, args.name, JSON.stringify(args.data), args.data.currencyCode]
